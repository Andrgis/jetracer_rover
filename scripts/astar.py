...
from geometry_msgs.msg import PoseStamped, Pose, Point, Quaternion, TwistStamped, Twist
...
def a_star(start,goal,grid,dist_map,res):
    ...
    open_list=[]; heapq.heappush(open_list,(0,start))
    closed=set();
    while open_list:
...
class AStarPlannerNode(object):
    ...
    def goal_cb(self,msg):
        # get start from AMCL
        start_msg=rospy.wait_for_message('amcl_pose',PoseStamped)
        sx,sy,sth = start_msg.pose.position.x/self.res, start_msg.pose.position.y/self.res, 2*math.atan2(start_msg.pose.orientation.z, start_msg.pose.orientation.w)
        gx,gy,gth = msg.pose.position.x/self.res, msg.pose.position.y/self.res, 2*math.atan2(msg.pose.orientation.z,msg.pose.orientation.w)
        start=Node(sx,sy,sth,0,None); goal=Node(gx,gy,gth,0,None)
        ...
        for n in path_states:
            ps=PoseStamped(header=ros_path.header)
            ps.pose.position=Point(n.x*self.res,n.y*self.res,0)
            ps.pose.orientation=Quaternion(0,0,math.sin(n.theta/2),math.cos(n.theta/2))
            ros_path.poses.append(ps)
        self.path_pub.publish(ros_path)
        # publish action sequence
        for i in range(len(path_states)-1):
            cur = path_states[i]; nxt = path_states[i+1]
            dx = (nxt.x-cur.x)*self.res; dy = (nxt.y-cur.y)*self.res
            theta = cur.theta
            # approximate forward and angular velocity
            twist = Twist()
